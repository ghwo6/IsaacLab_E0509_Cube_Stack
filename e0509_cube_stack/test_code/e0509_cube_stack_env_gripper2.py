...
"""
Doosan E0509 Cube Stack 강화학습 환경 v4
- 6축 로봇 + RH-P12-RN 그리퍼
- 그리퍼 1D 제어 (실제 로봇과 동일)
- 4개 관절 동기화 제어
...
class E0509CubeStackEnvCfg(DirectRLEnvCfg):
    """E0509 Cube Stack 환경 설정 v4"""

    # 환경 설정
    episode_length_s = 12.0
    decimation = 2
    action_scale = 1.5
    # 수정: 액션 공간 축소 (6 arm + 1 gripper = 7)
    action_space = 7
    # 수정: 관측 공간도 그리퍼를 1D로 (25 - 3 = 22, 또는 그대로 유지)
...
class E0509CubeStackEnv(DirectRLEnv):
    """
    E0509 로봇 Cube Stack 환경 v4
    
    주요 변경사항:
    1. 그리퍼 액션을 1D로 통합 (open/close)
    2. 4개 그리퍼 관절을 동기화하여 제어
    3. 실제 로봇과 동일한 제어 방식으로 Sim2Real 용이
    """
    ...
    def __init__(self, cfg: E0509CubeStackEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 디버그 출력
        # print(f"=== E0509 Cube Stack Env v4 ===")
        # print(f"Action space: {self.cfg.action_space} (6 arm + 1 gripper)")
        # print(f"Gripper open pos: {self.gripper_open_pos}")
        # print(f"Gripper close pos: {self.gripper_close_pos}")
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # 그리퍼 (1D → 4D 동기화)
        # 액션 범위 [-1, 1]을 그리퍼 위치 범위로 변환
        # -1 = open (lower limit), +1 = close (upper limit)
        gripper_action = self.actions[:, 6:7]  # [batch, 1]
        
        # 현재 그리퍼 타겟에 delta 적용
        gripper_delta = self.cfg.action_scale * self.cfg.sim.dt * gripper_action
        
        # 4개 관절 동기화 (실제 관절 순서: l1, r1, l2, r2)
        # l1(6)과 l2(8)는 같이, r1(7)과 r2(9)는 같이 움직여야 함
        gripper_indices = [6, 7, 8, 9]  # l1, r1, l2, r2
        for joint_idx in gripper_indices:
            current = self.robot_dof_targets[:, joint_idx:joint_idx+1]
            new_target = current + gripper_delta.squeeze(-1).unsqueeze(-1) * (self.robot_dof_upper_limits[joint_idx] - self.robot_dof_lower_limits[joint_idx])
            self.robot_dof_targets[:, joint_idx] = torch.clamp(
                new_target.squeeze(-1),
                self.robot_dof_lower_limits[joint_idx],
                self.robot_dof_upper_limits[joint_idx]
            )
    ...
    def _get_observations(self) -> dict:
        ...
        - gripper_open (1) = 1  # 수정: 4D → 1D
        - cubeA_relative (3) = 3
        - cubeA_to_cubeB (3) = 3  # 제거 가능하지만 유지
        
        실제: 7 + 3 + 7 + 1 + 3 + 1(finger_dist) = 22
        """
        ...
        gripper_closed_amount = gripper_pos[:, 0] + gripper_pos[:, 2]  # l1 + r1
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # 4. 박스를 잡았는지
        gripper_holding = (finger_distance > self.cubeA_width - 0.05) & (finger_distance < self.cubeA_width + 0.01)
        ...
        # ========== Stage 2: 그립 보상 ==========
        target_finger_dist = self.cubeA_width + 0.01
        
        # 박스가 사이에 있으면 닫기 보상 (grasp_ready 조건 제거)
        grip_error = torch.abs(finger_distance - target_finger_dist)
        grip_reward = box_between_fingers.float() * (1.0 - torch.tanh(20.0 * grip_error))
        # 추가: 닫는 동작 자체에 보상 (박스가 사이에 있을 때)
        closing_reward = box_between_fingers.float() * (1.0 - gripper_open / self.max_gripper_open) * 2.0
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        # 그리퍼 완전히 열기 (각 관절의 lower limit 사용)
