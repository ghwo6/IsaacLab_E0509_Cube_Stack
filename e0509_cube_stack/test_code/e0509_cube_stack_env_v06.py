...
class E0509CubeStackEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # l1(6), r1(7)만 제어 (l2, r2는 USD에서 고정됨)
        grip_ratio = 0.80  # 80% 닫기
        
        # l1 (index 6)
        open_pos_l1 = self.robot_dof_lower_limits[6]
        close_pos_l1 = self.robot_dof_upper_limits[6]
        partial_close_l1 = open_pos_l1 + grip_ratio * (close_pos_l1 - open_pos_l1)
        
        # r1 (index 7)
        open_pos_r1 = self.robot_dof_lower_limits[7]
        close_pos_r1 = self.robot_dof_upper_limits[7]
        partial_close_r1 = open_pos_r1 + grip_ratio * (close_pos_r1 - open_pos_r1)
        
        # l1 제어
        self.robot_dof_targets[:, 6] = torch.where(
            self.should_close_gripper,
            torch.full_like(self.robot_dof_targets[:, 6], partial_close_l1),
            torch.full_like(self.robot_dof_targets[:, 6], open_pos_l1),
        )
        
        # r1 제어
        self.robot_dof_targets[:, 7] = torch.where(
            self.should_close_gripper,
            torch.full_like(self.robot_dof_targets[:, 7], partial_close_r1),
            torch.full_like(self.robot_dof_targets[:, 7], open_pos_r1),
        )
        
        # l2, r2는 건드리지 않음 (USD에서 고정)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # 박스가 그리퍼 사이에 위치 (xy 거리 < 6cm, 높이 차이 < 6cm) - 조건 완화
